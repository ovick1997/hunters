...
<div id="comments" class="comments-area">
	...
	if ( have_comments() ) : ?>
		...
		<h2 class="comments-title">
			...
			if ( '1' === $comments_number ) {
				...
				printf( _x( 'One Reply to &ldquo;%s&rdquo;', 'comments title', 'hunters' ), get_the_title() );
			} else {
				printf(
		...
		<ul class="comment-list">
			<?php
				wp_list_comments( array(
					'avatar_size' => 60,
					// 'callback' => 'hunters_comment',
					'short_ping'  => true,
					//'reply_text'  => hunters_get_svg( array( 'icon' => 'mail-reply' ) ) . __( 'Reply', 'hunters' ),
				) );
			?>
		</ul>
		...
		the_comments_pagination( array(
			'prev_text' => '<span class="screen-reader-text">' . __( 'Before', 'hunters' ) . '</span>',
			'next_text' => '<span class="screen-reader-text">' . __( 'After', 'hunters' ) . '</span>',
		) );
	...
	if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) : ?>
		...
		<p class="no-comments"><?php _e( 'Comments are closed.', 'hunters' ); ?></p>
	<?php
	endif;
    ...
    $comments_args = array(
		...
		'fields' => apply_filters( 'comment_form_default_fields', array(
			...
			'email'  => '<div class="form-padding-right">' . '<input class="form-control" id="email" name="email" placeholder="' . esc_attr__( "E-mail*", "text-domain" ) . '" ' . ( $html5 ? 'type="email"' : 'type="text"' ) . ' size="30" maxlength="100" aria-describedby="email-notes"' . $aria_req . $html_req  . ' /></div>',
